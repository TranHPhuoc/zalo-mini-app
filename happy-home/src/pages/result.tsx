        ...
        if (paymentResult && paymentResult.resultCode) {
          if (paymentResult.resultCode === 1) {
            return render({
              ...
              message: `Chi phí dịch vụ căn hộ của Quý khách đã được thanh toán. Cảm ơn Quý khách đã sử dụng dịch vụ của chúng tôi!`,
              icon: <IconPaymentSuccess />,
            });
          } else {
            return render({
              title: "Thanh toán thất bại",
              message: `Có lỗi trong quá trình xử lý, vui lòng ấn thanh toán lại hoặc ấn nút gọi ngay để được hỗ trợ`,
